...
void depth_to_laser::depth_laser_fusion(sensor_msgs::LaserScanPtr scan_msg, std::vector<geometry_msgs::Pose2D> laserscan_points, const Params param)
{
/*
	...
	for (int i = 0; i < ranges_size; i++)
	{
		...
		// New point (In the robot coordinate)
		double robot_x = -kinect_r * sin(kinect_th);
		double robot_z =  kinect_r * cos(kinect_th);
	...
	depth_msg->range_min = range_min;    
	depth_msg->range_max = range_max;
*/
}
